...
if not exist "%GRAALVM_HOME%\bin\native-image.cmd" (
    echo %_ERROR_LABEL% Command native-image not found 1>&2
...
) else if %_VERBOSE%==1 ( echo Analyze %__N_FILES% with CheckStyle configuration "!__XML_FILE:%USERPROFILE%\=%%USERPROFILE%%!" 1>&2
)
...
set __NATIVE_IMAGE_OPTS=-cp "%_CLASSES_DIR%" --no-fallback --language:js
...
set __TEST_MAIN_CLASS=MainJUnitTest

call :libs_cpath
if not %_EXITCODE%==0 goto :eof

set "__CPATH=%_LIBS_CPATH%;%_CLASSES_DIR%;%_TEST_CLASSES_DIR%"
set __TEST_JAVA_OPTS=-classpath "%__CPATH:\=\\%"

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_JAVA_CMD%" %__TEST_JAVA_OPTS% org.junit.runner.JUnitCore %__TEST_MAIN_CLASS% 1>&2
) else if %_VERBOSE%==1 ( echo Execute JMH benchmark ^(JVM^) 1>&2
)
call "%_JAVA_CMD%" %__TEST_JAVA_OPTS% org.junit.runner.JUnitCore %__TEST_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
    set _EXITCODE=1
    goto :eof
)
